...
#define WIDTH_HEIGHT_ODD 3      // Both the width and the height of the texture are odd.

struct ComputeShaderInput
{
    uint3 GroupID           : SV_GroupID;           // 3D index of the thread group in the dispatch.
    uint3 GroudThreadID     : SV_GroupThreadID;     // 3D index of the local thread ID in a thread group.
...
{
    uint SrcMipLevel;   // Texture level of the source mip.
    uint NumMipLevels;  // Number of OutMips to write: [1-4]
    uint SrcDimension;  // Width and height of the source texture are even of odd.
    bool IsSRGB;        // Must apply gamma correction to sRBG textures.
...
// Convert linear color to sRGB before storing 
// if the the original source is an sRGB texture.
...
void main(ComputeShaderInput IN)
{
    ...
    switch (SrcDimension)
    {
        ...
        {
            // > 2:1 in X dimension
            // Use 2 bilinear samples to guarantee we don't undersample 
            // when downsizing by more than 2x horizonatally.
        ...
        case WIDTH_EVEN_HEIGHT_ODD:
        {
            // > 2:1 in Y dimension
            // Use 2 bilinear samples to guarantee we don't undersample
            // when downsizing by more than 2x vertically.
        ...
        case WIDTH_HEIGHT_ODD:
        {
            ...
            // Use 4 bilinear samples to guarantee we don't undersample
            // when downsizing by more than 2x in both directions.
    ...
    // This guarantees all LDS writes are complete and that all threads executed 
    // all instructions so far (and therefore have issued their LDS write instructions).
    GroupMemoryBarrierWithGroupSync();
    
    // With low three bits for X and high three bits for Y,
    // this bit mask (binary: 001001) checks that X and Y are even.
