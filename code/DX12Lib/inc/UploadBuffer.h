...
public:
	...
	*/
	...
	/*
	* Release all allocated pages. This should only be done when
	* the cammand list is finished executing on the CommandQueue.
	*/
	void Reset();
...
private:
	...
	*/
	struct Page
	{
		...
		/*
		* Check to see if the page has room to satisfy the requester allocation.
		*/
		bool HasSpace(size_t sizeInBytes, size_t alignment) const;

		/*
		* Allocate memory from the page.
		* Throws std::bad_alloc if the allocation size is larger
		* than the page size or the size of the allocation exceeds
