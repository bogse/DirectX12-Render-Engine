...
DescriptorAllocation DescriptorAllocatorPage::Allocate(uint32_t numDescriptors)
{
	...
	// The size of the smallest block that satisfy the request.
	SizeType blockSize = smallestBlockIt->first;

	// The pointer to the same entru in the FreeListByOffset map.
	FreeListByOffset::iterator offsetIt = smallestBlockIt->second;

	// The offset in the descriptor heap.
	OffsetType offset = offsetIt->first;
...
void DescriptorAllocatorPage::FreeBlock(uint32_t offset, uint32_t numDescriptors)
{
	...
	FreeListByOffset::iterator nextBlockIt = m_FreeListByOffset.upper_bound(offset);

	// Find the block that appears before the block being freed.
	FreeListByOffset::iterator prevBlockIt = nextBlockIt;
	...
	{
		// Otherwise, just set it to the end of the list to indicate
		// that no block comes before the one being freed.
	...
	// blocks modifies the numDescriptors varible.
	m_NumFreeHandles += numDescriptors;
