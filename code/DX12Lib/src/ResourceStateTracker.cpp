...
void ResourceStateTracker::FlushResourceBarriers(CommandList& commandList)
{
	...
	if (numBarriers > 0)
	{
		Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList2> d3d12CommandList = commandList.GetGraphicsCommandList();
	...
	for (D3D12_RESOURCE_BARRIER pendingBarrier : m_PendingResourceBarriers)
	{
		if (pendingBarrier.Type == D3D12_RESOURCE_BARRIER_TYPE_TRANSITION) // Only transition barriers should be pending.
		{
			...
			if (iter != ms_GlobalResourceState.end())
			{
				...
				{
					// Fix-up the before state based on the current global state of the resource.
