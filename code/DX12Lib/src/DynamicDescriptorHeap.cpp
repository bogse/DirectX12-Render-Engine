	...
	D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptor)
{
	if (!m_CurrentDescriptorHeap || m_NumFreeHandles < 1)
	{
		...
		// When updating the descriptor heap on the command list, all descriptor tables must be
		// (re)recopied to the new descriptor heap (not just the stale descriptor tables).
		m_StaleDescriptorTableBitMask = m_DescriptorTableBitMask;
	}
