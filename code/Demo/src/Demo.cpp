	...
	D3D12_RESOURCE_FLAGS flags)
{
	...
	CD3DX12_RESOURCE_DESC defaultDesc = CD3DX12_RESOURCE_DESC::Buffer(bufferSize, flags);
	...
	// Create a comitted resource for the upload.
	if (bufferData)
	{
...
bool Demo::LoadContent()
{
	...
	UpdateBufferResource(commandList.Get(),
		...
		_countof(g_Indices), sizeof(WORD), g_Indices);
...
void Demo::OnResize(ResizeEventArgs& eventArgs)
{
	if (eventArgs.m_Width != GetClientWidth() || eventArgs.m_Height != GetClientHeight())
	{
		...
		m_Viewport = CD3DX12_VIEWPORT(0.0f, 0.0f,
			static_cast<float>(eventArgs.m_Width),
